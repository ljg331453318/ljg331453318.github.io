...
function setLanguage(lang){
  ...
  const profile=document.querySelector('.profile'); profile.classList.toggle('en-mode',lang==='en');
  document.querySelector('.profile-text').innerHTML=lang==='en'
    ? `<a class="name-link" href="#home">Jiageng Liu</a><p class="profile-role">Associate Professor, Master’s Supervisor<br />Electrical Engineer, Senior Electrical Technician, Psychological Counselor</p><p class="profile-affiliation">School of Equipment Engineering, Shenyang Ligong University<br />Full-time Faculty, Information Confrontation</p>`
    : `<a class="name-link" href="#home">${t.site}</a><p>${t.profile}</p><p>${t.profileSchool}</p>`;
  ...
  document.querySelector('#publications .section-note').textContent=t.pubNote; document.querySelector('#patents .section-note').textContent=t.patentNote; updatePublicationDetails(lang);
  if(lang==='en')setList('#patents .patent-list',patentsEn.map((x,i)=>`<span>${['2023','2023','2024','2026','2026'][i]}</span><a href="${document.querySelectorAll('#patents .patent-list a')[i]?.href||'#'}" target="_blank" rel="noopener">${x}</a><small>Granted Chinese invention patent · Granted · ${i<2?'First inventor':'Inventor'}</small>`));
  else {const patentTitles=['一种复杂受限环境中车辆融合定位系统及方法','一种基于残差检测的车辆定位完好性监测方法及系统','一种基于场景分类的自动驾驶车辆导航方法','自动驾驶定位完好性故障检测识别自适应评估方法','隧道环境下使用长期演进接收信号强度指示符的定位方法']; const urls=['CN114415224B','CN115291253B','CN115285143B','CN121384095B','CN117930132B']; setList('#patents .patent-list',patentTitles.map((x,i)=>`<span>${['2023','2023','2024','2026','2026'][i]}</span><a href="https://patents.google.com/patent/${urls[i]}/zh" target="_blank" rel="noopener">${x}</a><small>中国发明专利 · 已授权 · ${urls[i].replace('B',' B')} · ${i<2?'第一发明人':'发明人'}</small>`))}
  ...
  document.querySelector('#contact').innerHTML=`<h2>${t.contactTitle}</h2><p>${t.address}</p><p>${t.email}<a href="mailto:liu331453318@163.com">liu331453318@163.com</a><br /><a href="mailto:jiagengliu@sylu.edu.cn">jiagengliu@sylu.edu.cn</a></p>`; document.querySelector('footer').textContent=t.footer;
